<?php
	...
	*/
	...
	$_SG['banco'] = 'bd_escola';            // Banco de dados MySQL
	*/
	$_SG['servidor'] = 'www.terenciani.com.br';    				// Servidor MySQL
	$_SG['usuario'] = 'teren671_admin';          // Usuсrio MySQL
	...
	$_SG['tabela'] = 'tb_administrador';       // Nome da tabela onde os usuсrios sуo salvos
	// ==============================
	...
	if ($_SG['conectaServidor'] == true) {
		$_SG['link'] = mysqli_connect($_SG['servidor'], $_SG['usuario'], $_SG['senha']) or die("MySQL: Nуo foi possэvel conectar-se ao servidor [".$_SG['servidor']."].");
		mysqli_select_db($_SG['banco'], $_SG['link']) or die("MySQL: Nуo foi possэvel conectar-se ao banco de dados [".$_SG['banco']."].");
	}
...
function validaUsuario($usuario, $senha) {
	...
	$sql = "SELECT `id_administrador`, `nome` FROM `".$_SG['tabela']."` WHERE ".$cS." `usuario` = '".$nusuario."' AND ".$cS." `senha` = '".$nsenha."' LIMIT 1";
